export const GENDER_OPTIONS = [
  { label: 'Male', value: 'Male' },
  { label: 'Female', value: 'Female' },
  { label: 'Prefer not to say', value: 'Prefer not to say' },
];

export const OCCUPATION_OPTIONS = [
  {
    label: 'Business / Entrepreneur',
    value: 'Business / Entrepreneur',
  },
  {
    label: 'Self-Employed / Freelancer',
    value: 'Self-Employed / Freelancer',
  },
  {
    label: 'Job / Working Professional',
    value: 'Job',
  },
  {
    label: 'Trainer / Speaker / Coach / Author',
    value: 'Trainer / Speaker / Coach / Author',
  },
  {
    label: 'Housewife / Homemaker',
    value: 'Housewife',
  },
  {
    label: 'Student',
    value: 'Student',
  },
];

export const STRUGGLING_AREAS_OPTIONS = [
  { label: 'Money & Finance', value: 'Money & Finance' },
  { label: 'Business & Revenue', value: 'Business' },
  { label: 'Career Growth', value: 'Career' },
  { label: 'Health & Vitality', value: 'Health' },
  { label: 'Relationships & Harmony', value: 'Relationship' },
  { label: 'Self Growth & Discipline', value: 'Self Growth' },
  { label: 'Energy, Passion & Drive', value: 'Energy & Passion' },
];

export const OBSTACLE_OPTIONS = [
  { label: 'Lack of Support', value: 'Lack of Support' },
  { label: 'Lack of Health & Energy', value: 'Lack of Health & Energy' },
  { label: 'Lack of Skill & Knowledge', value: 'Lack of Skill & Knowledge' },
  { label: 'Lack of Right Mindset', value: 'Lack of Right Mindset' },
  { label: 'Lack of Strategy & Plan', value: 'Lack of Strategy & Plan' },
  { label: 'Other', value: 'Other' },
];

export const INVESTMENT_OPTIONS = [
  {
    label: 'A) ₹5,000 to ₹10,000',
    value: 'A) 5k to 10k',
  },
  {
    label: 'B) ₹15,000 to ₹25,000',
    value: 'B) 15k to 25k',
  },
  {
    label: 'C) ₹25,000 to ₹45,000',
    value: 'C) 25k to 45k',
  },
  {
    label: 'D) Ready To Invest Any Amount, If I Get A Result',
    value: 'D) Ready To Invest Any Amount, If I Get A Result',
  },
  {
    label: 'E) No, I cannot invest at this time',
    value: "E) No I can't Invest",
  },
];
